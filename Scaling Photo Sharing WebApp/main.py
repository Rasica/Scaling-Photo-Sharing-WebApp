...
_INSTANCE_NAME = 'consummate-rush-97119:assignment2'
  ...
  def signUp(self,username,password,cnpassword):
    try:
        if (os.getenv('SERVER_SOFTWARE') and
            ...
            db = MySQLdb.connect(unix_socket='/cloudsql/' + _INSTANCE_NAME, db='cloudSQL1', user='root')
        else:
            db = MySQLdb.connect(host='173.194.226.229', port=3306, db='cloudSQL1', user='abhiUTA',passwd='abhi123')
        cursor = db.cursor()
...
class ThreadPage(webapp2.RequestHandler): 
     ...
     def post(self):
            bucket_name = 'cloudbucket786'#Bucket Name
            bucket = '/' + bucket_name
     ...
     def write_file(self,fileName,name):
              ...
              url='https://console.developers.google.com/m/cloudstorage/b/cloudbucket786/o/'+name
